...
export abstract class Enemies {
    abstract gameWinner(): void
    abstract gameInterval(): Promise<string>
    abstract processGame(options: string): Promise<void>
    ...
    async attackOnEnemy(enemy: IEnemy): Promise<void> {
    ...
    generateReciept(enemy: IEnemy): void {
        console.log(`
        ${chalk.cyan('You successfully destroyed your enemy.')}
        ${enemy.died.map((d, i) => 
            chalk.yellowBright(`    ${i+1}: ${d}`)
            )}
        \n
         ${chalk.cyan('Your remaining Enemies')}
         ${enemy.opponents.map((e, i) => 
             chalk.yellowBright(`   ${i+1}: ${e}`)
            )}
        `)
        console.log(chalk.bgBlue(`
